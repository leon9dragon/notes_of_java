...
- [3. Lock 机制和 Condition 接口实现线程通信](#3-lock-机制和-condition-接口实现线程通信)
  ...
  - [3.2. Condition 接口](#32-condition-接口)
  - [3.3. 用锁机制来实现消费者案例的线程通信](#33-用锁机制来实现消费者案例的线程通信)
...
- 其中, `Lock` 接口替代了 `synchronized` 方法和语句,  
  `Condition` 接口则替代了 `Object` 类中的监视器方法.



### 3.3. 用锁机制来实现消费者案例的线程通信
```java
//共享资源对象(姓名-性别) 
public class ShareResource {
    private String name;
